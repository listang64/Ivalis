...
window.genererProfilDeck = async function() {
    ...
    try {
        ...
        // NOUVEAU : On délègue tout l'affichage au nouveau module de gestion !
        if (typeof window.afficherDeckInteractif === "function") {
            window.afficherDeckInteractif(idPersonnage, deckProcedural, couleurPerso, profilJson.Theme_Identifie || "Deck Généré");
        }
...
window.chargerDeckExistant = async function(idPersonnage) {
    ...
    try {
        ...
        if (deckSnap.exists()) {
            ...
            // NOUVEAU : On délègue tout l'affichage au nouveau module de gestion !
            if (typeof window.afficherDeckInteractif === "function") {
                window.afficherDeckInteractif(idPersonnage, deckProcedural, couleurPerso, donneesDeck.Donnees_IA?.Theme_Identifie || "Deck du Héros");
            }
