...
// LES DEUX OBJETS DU PREMIER JOUR, DESSINÉS AVANT LE HÉROS.
//  L'arme et la tenue partent ensemble chez le dessinateur (MIA_Objets les
//  décrit d'une seule requête, les images se font en parallèle). Seule la tenue
//  servira de référence au portrait ; l'arme est illustrée pour l'inventaire.
//
//  Un échec ici n'empêche jamais la création : on repart alors sur l'ancien
//  chemin, le héros habillé d'après le seul nom de sa tenue.
async function illustrerEquipementDeDepart(donnees) {
  if (typeof window.illustrerLesObjets !== "function") return;
  if (typeof window.peutIllustrerLesObjets !== "function" || !window.peutIllustrerLesObjets()) return;

  const aDessiner = [donnees.equipArmure, donnees.equipMainDroite, donnees.equipMainGauche]
      .filter(o => o && o.uid && !o.image)
      // Une arme à deux mains occupe deux emplacements avec le même objet.
      .filter((o, i, tous) => tous.findIndex(x => x.uid === o.uid) === i);
  if (aDessiner.length === 0) return;
  ...
  // illustrerLesObjets pose l'URL sur l'objet lui-même ; les deux mains d'une
  // arme à deux mains pointent sur le même objet, rien à recopier.
  if (donnees.equipArmure && donnees.equipArmure.image) {
    donnees.imageArmureReference = donnees.equipArmure.image;
  }
  annoncerEtapeCreation("Création de personnage en cours ...");
}

// Le texte de l'écran de chargement : la création prend maintenant deux temps,
// autant dire lequel est en cours plutôt que laisser le joueur devant un écran
// figé pendant une minute.
function annoncerEtapeCreation(texte) {
  const titre = document.getElementById("titre-chargement-ia");
  if (titre) titre.innerText = texte;
}
...
async function sauvegarderFichePersonnage(donnees, skipImage = false) {
  ...
  if (estNouveau) {
    ...
    // L'ÉQUIPEMENT EST DESSINÉ AVANT LE HÉROS.
    //  Il faut l'image de la tenue pour la joindre au portrait : sans elle, le
    //  héros naîtrait vêtu d'une armure inventée, et il faudrait le redessiner
    //  juste après — deux images, et un personnage qui change sous les yeux du
    //  joueur. L'arme, elle, est illustrée en même temps : elle ne sert pas au
    //  portrait, mais elle sera montrée dans l'inventaire.
    if (!skipImage) await illustrerEquipementDeDepart(donnees);
  }
...
async function genererEtStockerPortrait(donnees) {
  ...
  if (donnees.style) {
      descriptionHero += donnees.imageArmureReference
        // L'image de la tenue est jointe : elle fait foi, le nom ne sert qu'à
        // la nommer. Dire au modèle de la RECOPIER vaut mieux que la décrire.
        ? `Il porte la tenue montrée sur l'image de référence jointe (${donnees.style}) : `
          + "reprends-en fidèlement la coupe, les matières, les couleurs et les ornements, "
          + "ajustés sur son corps. Elle doit être bien visible et reconnaissable. "
        : `Il porte cette tenue complète, et elle doit être bien visible : ${donnees.style}. `
  ...
  //  Avec l'image de la tenue de départ sous la main, on passe par l'ÉDITION et
  //  on la joint en binaire : le héros naît habillé de l'armure exacte qu'il
  //  porte sur sa fiche, en une seule image. Une URL glissée dans le prompt
  //  serait purement et simplement ignorée par le modèle — seuls les pixels
  //  comptent, c'est la même leçon que pour les pions.
  //  Sans elle (dev, clé absente, tirage sans armure), on retombe sur la
  //  génération pure d'avant.
  const blobTenue = donnees.imageArmureReference && typeof window.imageVersBlobPng === "function"
      ? await window.imageVersBlobPng(donnees.imageArmureReference).catch(() => null)
      : null;

  const urlOpenAI = blobTenue
      ? "https://api.openai.com/v1/images/edits"
      : "https://api.openai.com/v1/images/generations";

  // La taille éprouvée des portraits du jeu : tous les héros existants sont
  // dans ce format, en changer ferait cohabiter deux silhouettes. Si l'API la
  // refuse sur l'endpoint d'édition, on bascule sur le seul autre format
  // portrait qu'elle admette — et on le dit en console plutôt que de rendre un
  // portrait manquant.
  let taillePortrait = window.TAILLE_PORTRAIT_IVALIS || "1024x1792";
  let optionsOpenAI;
  if (blobTenue) {
    const form = new FormData();
    form.append("model", "gpt-image-2");
    form.append("prompt", promptOpenAI);
    form.append("image[]", blobTenue, "tenue.png");
    form.append("n", "1");
    form.append("size", taillePortrait);
    form.append("quality", "medium");
    form.append("output_format", "png");
    // Aucun en-tête Content-Type : le navigateur pose lui-même la frontière multipart.
    optionsOpenAI = { method: "POST", headers: { "Authorization": "Bearer " + cles.openai }, body: form };
  } else {
  const payloadOpenAI = {
    model: "gpt-image-2",
    prompt: promptOpenAI,
  ...
  // Rejouer la construction des options avec la taille courante : c'est ce qui
  // permet de réessayer dans un autre format sans dupliquer le payload.
  const reconstruireOptions = () => {
    if (!blobTenue) {
      const p = JSON.parse(optionsOpenAI.body);
      p.size = taillePortrait;
      return { ...optionsOpenAI, body: JSON.stringify(p) };
    }
    const form = new FormData();
    form.append("model", "gpt-image-2");
    form.append("prompt", promptOpenAI);
    ...
    form.append("quality", "medium");
    form.append("output_format", "png");
    return { method: "POST", headers: { "Authorization": "Bearer " + cles.openai }, body: form };
  };

  // --- Systeme belier (anti-spam 1015) : 3 tentatives 5s / 15s / 30s ---
  let tentative = 0;
  let succes = false;
  let repliTaille = false;
  ...
  while (tentative < 3 && !succes) {
    ...
    if (texteReponseOpenAI.includes("error code: 1015") || texteReponseOpenAI.includes("Rate Limited")) {
      ...
      tentative++;
    } else if (!repliTaille && /size/i.test(texteReponseOpenAI) && /error/i.test(texteReponseOpenAI)) {
      // Le format habituel n'est pas accepté ici : on reprend au seul autre
      // format portrait que le modèle admette, plutôt que de rendre un héros
      // sans visage.
      repliTaille = true;
      console.warn("Format " + taillePortrait + " refusé par l'API, nouvelle tentative en 1024x1536.");
      taillePortrait = "1024x1536";
      optionsOpenAI = reconstruireOptions();
    } else {
      succes = true;
    }
  }
